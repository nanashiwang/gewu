...
| Enterprise Registry | 已有 | `EnterpriseRegistries`、`/v1/enterprise/overview`、`/v1/enterprise/registry/[id]/passport`、`/v1/enterprise/registry/[id]/benchmark`、`/v1/enterprise/registry/review-required`，返回治理总览、治理 checklist、组织内证书/Passport、采用基线漂移告警、私有评测、批量重审、审计导出和企业失败库入口 |
