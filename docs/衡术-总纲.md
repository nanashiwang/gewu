...
企业身份策略已提供域名白名单、OIDC/SSO、SCIM 配置校验和接入 playbook；SCIM provision 已支持成员创建、查询、停用和 ListResponse；OIDC SSO 已具备 authorize 发起包、HMAC state 校验、callback 组织上下文还原、服务端 tokenExchange 请求包、ID Token claims 校验和 active 成员绑定预检；JWKS 签名校验与正式会话签发仍是后续接入项。
