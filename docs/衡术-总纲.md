...
当前 FailureCase 已支持“自动聚类 → 前台归因看板 → 人工归因/复验覆盖 → 私人台账复验计划 → 批量复验队列 → 公开脱敏复核”的闭环；公开 API 输出脱敏画像、人工归因摘要、复验覆盖、triage checklist、私人台账复现入口、模型画像、Adapter、证据验签入口和登录后的 reverify-plan；登录用户可把计划放入 reverify-queue，按 failureCaseId+userId 去重，不暴露原始输入输出。
...
| FailureCase      | 已有   | `FailureCases`、`/v1/failures`、`/v1/failures/[id]/triage`、`/v1/failures/[id]/reverify-plan`、`/v1/failures/[id]/reverify-queue`、`/console/failures/triage`，公开脱敏画像、归因看板、人工归因摘要、复验覆盖、triage checklist、台账复现、自动复验计划、批量复验入队和证据验签入口 |
...
| 失败知识           | 已有一等 `FailureCase`、归因看板、人工归因、复验覆盖、Adapter 草稿链路、公开复验入口、私人台账 reverify-plan 和 Redis 批量入队接口 | 后续补自动复验 worker 与结果回写           |
