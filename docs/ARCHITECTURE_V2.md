...
| FailureCase | `src/collections/FailureCases.ts`、`src/lib/failureKnowledge.ts`、`src/lib/failureRefresh.ts`、`src/lib/failureCasePublic.ts` | 按 Skill × 输入档 × errorType 聚合任务失败画像；随失败回流自动刷新并写证据快照；原始集合仅后台可读，公开读取走脱敏失败库 API，并返回人工归因摘要、复验覆盖、triage checklist、私人台账复现、模型画像、Adapter 和证据验签入口。 |
